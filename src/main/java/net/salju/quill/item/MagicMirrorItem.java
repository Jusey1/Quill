...
public class MagicMirrorItem extends Item {
	...
	public void inventoryTick(ItemStack stack, Level world, Entity target, int e, boolean check) {
		...
		if (target instanceof ServerPlayer ply && world instanceof ServerLevel) {
			if (ply.getRespawnPosition() == null) {
				if (!BlockPos.containing(data.getPos().getX(), data.getPos().getY(), data.getPos().getZ()).equals(world.getLevelData().getSpawnPos())) {
					stack.set(QuillData.TELEPORT, new MagicMirrorTeleport(Optional.of(GlobalPos.of(Level.OVERWORLD, world.getLevelData().getSpawnPos()))));
				}
			} else {
				if (!BlockPos.containing(data.getPos().getX(), data.getPos().getY(), data.getPos().getZ()).equals(ply.getRespawnPosition())) {
					stack.set(QuillData.TELEPORT, new MagicMirrorTeleport(Optional.of(GlobalPos.of(ply.getRespawnDimension(), ply.getRespawnPosition()))));
				}
			}
		}
	}
	...
	public boolean releaseUsing(ItemStack stack, Level world, LivingEntity target, int i) {
		...
		if (i <= 10 && target instanceof ServerPlayer ply && world instanceof ServerLevel lvl && data.target().isPresent()) {
			...
			double y = data.getPos().getY() + (ply.getRespawnPosition() != null ? 0.7 : 0.0);
